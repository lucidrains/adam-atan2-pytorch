...
class AdamAtan2ClipGrok(Optimizer):
    def __init__(
        self,
        params,
        lr = 1e-4,
        ...
        defaults = dict(
            ...
            clip_to_grok = clip_to_grok,
            clip_to_grok_max_norm = clip_to_grok_max_norm
        )
    ...
    def step(
        ...
        for group in self.param_groups:
            for p in filter(lambda p: exists(p.grad), group['params']):
                ...
                grad, lr, wd, regen_rate, cautious_wd, cautious_factor, clip_to_grok, clip_to_grok_max_norm, beta1, beta2, a, b, state, init_lr = p.grad, group['lr'], group['weight_decay'], group['regen_reg_rate'], group['cautious_wd'], group['cautious_factor'], group['clip_to_grok'], group['clip_to_grok_max_norm'], *group['betas'], group['a'], group['b'], self.state[p], self._init_lr
